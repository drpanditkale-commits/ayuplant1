...
import { getPlantInfo, generatePlantImage } from './services/geminiService.js';
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      setState(prev => ({ 
        ...prev, 
        isLoading: false, 
        error: "We couldn't find information for that plant. Please try a more specific name." 
      }));
    }
  };
      ...
      <header className="relative bg-emerald-900 py-20 px-4 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center space-y-6">
          ...
          <p className="text-emerald-100/70 text-lg md:text-xl max-w-2xl mx-auto font-light leading-relaxed">
            Discover the ancient wisdom of medicinal herbs.
          </p>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 -mt-10">
        <div className="space-y-12">
          ...
          <div className="min-h-[400px]">
            ...
            ${!state.data && !state.isLoading && !state.error && html`
              <div className="flex flex-col items-center justify-center py-24 text-center text-stone-300">
                ...
                <p className="text-xl font-serif italic text-stone-400">Search for herbs like 'Tulsi' to begin.</p>
              </div>
            `}

            ${state.data && html`<${PlantCard} data=${state.data} imageUrl=${state.imageUrl} />`}
          </div>
        </div>
      </main>

      <footer className="mt-20 border-t border-stone-200 py-10 text-center">
        <p className="text-stone-400 text-sm">&copy; ${new Date().getFullYear()} AyurPlant Explorer</p>
      </footer>
    </div>
  `;
};
